...
TESTING_INTERVAL = 1000 # testing interval is first 10 time steps/frames
NO_FLAP = 0
FLAP = 1
BOUNDARY_TOLERANCE = 10 # epsilon for our objective to be met
OPT_STEP = 1e-6
...
def experimentation_window(current_time_step):
    if current_time_step < TESTING_INTERVAL:
        return False
    else:
        return True
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    # optimization_space/definitions
    ...
    while "flappy bird" != "angry bird":
        ...
        if experimentation_window:
            optimization_subject = list(D)
            # freeze weights
            Q_vals = readout.eval(feed_dict = {s : optimization_subject})
            optimizer.run(feed_dict={Q_val_a : Q_vals[:,NO_FLAP],
                                     Q_val_b : Q_vals[:,FLAP]})
